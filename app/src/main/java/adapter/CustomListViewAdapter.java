...
public class CustomListViewAdapter {
    ...
    // CustomListViewAdapter 생성자. 마지막에 ListBtnClickListener 추가.
    CustomListViewAdapter(Context context, int resource, ArrayList<ListViewItem> list, ListBtnClickListener clickListener) {
        super(context, resource, list) ;

        // resource id 값 복사. (super 로 전달된 resource 를 참조할 방법이 없음.)
        this.resourceId = resource ;

        this.listBtnClickListener = clickListener ;
    }

    // 새롭게 만든 Layout 을 위한 View 를 생성
    @Override
  ...
  /*      }
        ...
        final TextView textTextView = convertView.findViewById(R.id.textView1);
